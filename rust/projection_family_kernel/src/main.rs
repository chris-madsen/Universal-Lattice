...
fn run_server() {
    ...
    for line in stdin.lock().lines() {
        ...
        let value: Value = match serde_json::from_str(&line) {
            Ok(v) => v,
            Err(err) => {
                let _ = writeln!(stdout, "{}", serde_json::to_string(&json!({"error": format!("parse_error: {}", err)})).unwrap());
                let _ = stdout.flush();
                continue;
            }
        };
        let input: KernelInput = match serde_json::from_value(value) {
            Ok(v) => v,
            Err(err) => {
                let _ = writeln!(stdout, "{}", serde_json::to_string(&json!({"error": format!("schema_error: {}", err)})).unwrap());
                let _ = stdout.flush();
                continue;
            }
        };
        let output = evaluate(&input);
        let _ = writeln!(stdout, "{}", serde_json::to_string(&json!(output)).expect("failed to encode output"));
        let _ = stdout.flush();
    }
}
...
fn main() {
    ...
    let input_path = env::args().nth(1).map(PathBuf::from).expect("usage: projection_family_kernel <input.json> or projection_family_kernel --server");
